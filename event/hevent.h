...
// prepares
#if ENABLE_PREPARE
    struct list_head prepares;
    uint32_t         nprepares;
#endif   // ENABLE_PREPARE
// checks
#if ENABLE_CHECK
    struct list_head checks;
    uint32_t         nchecks;
#endif   // ENABLE_CHECK
    // timers
...
#define PREPARE_ENTRY(p) container_of(p, hprepare_t, node)
#define CHECK_ENTRY(p)   container_of(p, hcheck_t, node)
