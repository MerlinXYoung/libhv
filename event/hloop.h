...
#if ENABLE_PREPARE
typedef struct hprepare_s hprepare_t;
#endif
#if ENABLE_CHECK
typedef struct hcheck_s hcheck_t;
#endif
...
#if ENABLE_PREPARE
typedef void (*hprepare_cb)(hprepare_t* prepare);
#endif
#if ENABLE_CHECK
typedef void (*hcheck_cb)(hcheck_t* check);
#endif
...
typedef enum
{
    ...
    HEVENT_TYPE_PREPARE = 0x00000101,
    HEVENT_TYPE_CHECK   = 0x00000102,
...
// prepare
#if ENABLE_PREPARE
HV_EXPORT hprepare_t* hprepare_add(hloop_t* loop, hprepare_cb cb,
                                   uint32_t repeat DEFAULT(INFINITE));
HV_EXPORT void        hprepare_del(hprepare_t* prepare);
#endif

// check
#if ENABLE_CHECK
HV_EXPORT hcheck_t* hcheck_add(hloop_t* loop, hcheck_cb cb, uint32_t repeat DEFAULT(INFINITE));
HV_EXPORT void      hcheck_del(hcheck_t* check);
#endif

// timer
HV_EXPORT htimer_t* htimer_add(hloop_t* loop, htimer_cb cb, uint32_t timeout_ms,
                               uint32_t repeat DEFAULT(INFINITE));
/*
 ...
 */
HV_EXPORT htimer_t* htimer_add_period(hloop_t* loop, htimer_cb cb, int8_t minute DEFAULT(0),
                                      int8_t hour DEFAULT(-1), int8_t day DEFAULT(-1),
                                      int8_t week DEFAULT(-1), int8_t month DEFAULT(-1),
                                      uint32_t repeat DEFAULT(INFINITE));
...
// NOTE: hio_write is non-blocking, so there is a write queue inside hio_t to cache unwritten data
...
HV_EXPORT void        hio_set_connect_timeout(hio_t*         io,
                                              int timeout_ms DEFAULT(HIO_DEFAULT_CONNECT_TIMEOUT));
...
HV_EXPORT void hio_set_keepalive_timeout(hio_t*         io,
                                         int timeout_ms DEFAULT(HIO_DEFAULT_KEEPALIVE_TIMEOUT));
/*
...
HV_EXPORT hio_t* hwrite(hloop_t* loop, int fd, const void* buf, size_t len,
                        hwrite_cb write_cb DEFAULT(NULL));
// hio_get -> hio_close
...
// hio_get -> hio_setcb_write -> hio_write
HV_EXPORT hio_t* hsend(hloop_t* loop, int connfd, const void* buf, size_t len,
                       hwrite_cb write_cb DEFAULT(NULL));
...
// hio_get -> hio_setcb_write -> hio_write
HV_EXPORT hio_t* hsendto(hloop_t* loop, int sockfd, const void* buf, size_t len,
                         hwrite_cb write_cb DEFAULT(NULL));
...
// @tcp_server: hio_create_socket(loop, host, port, HIO_TYPE_TCP, HIO_SERVER_SIDE) ->
// hio_setcb_accept -> hio_accept
// @see examples/tcp_echo_server.c
HV_EXPORT hio_t* hloop_create_tcp_server(hloop_t* loop, const char* host, int port,
                                         haccept_cb accept_cb);

// @tcp_client: hio_create_socket(loop, host, port, HIO_TYPE_TCP, HIO_CLIENT_SIDE) ->
// hio_setcb_connect -> hio_setcb_close -> hio_connect
// @see examples/nc.c
HV_EXPORT hio_t* hloop_create_tcp_client(hloop_t* loop, const char* host, int port,
                                         hconnect_cb connect_cb, hclose_cb close_cb);

// @ssl_server: hio_create_socket(loop, host, port, HIO_TYPE_SSL, HIO_SERVER_SIDE) ->
// hio_setcb_accept -> hio_accept
// @see examples/tcp_echo_server.c => #define TEST_SSL 1
HV_EXPORT hio_t* hloop_create_ssl_server(hloop_t* loop, const char* host, int port,
                                         haccept_cb accept_cb);

// @ssl_client: hio_create_socket(loop, host, port, HIO_TYPE_SSL, HIO_CLIENT_SIDE) ->
// hio_setcb_connect -> hio_setcb_close -> hio_connect
// @see examples/nc.c => #define TEST_SSL 1
HV_EXPORT hio_t* hloop_create_ssl_client(hloop_t* loop, const char* host, int port,
                                         hconnect_cb connect_cb, hclose_cb close_cb);
...
// @tcp_upstream: hio_create_socket -> hio_setup_upstream -> hio_connect -> on_connect ->
// hio_read_upstream
// @return upstream_io
...
typedef struct unpack_setting_s
{
    ...
    union
    {
         ...
         *
         * if (length_field_coding == ENCODE_BY_VARINT) head_len = body_offset + varint_bytes -
         * length_field_bytes; else head_len = body_offset;
         *
