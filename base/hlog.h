...
#ifndef __FILENAME__
// #define __FILENAME__  (strrchr(__FILE__, DIR_SEPARATOR) ? strrchr(__FILE__, DIR_SEPARATOR) + 1 :
// __FILE__)
...
#if 1
#    define _hlog_lvl(lvl, fmt, ...)                                                                                                   \
        do {                                                                                                                           \
            logger_t* LL = hlog;                                                                                                       \
            if (LL && lvl >= LL->level) logger_print(LL, lvl, fmt " [%s:%d:%s]", ##__VA_ARGS__, __FILENAME__, __LINE__, __FUNCTION__); \
        } while (0)

#    define hlogd(fmt, ...) _hlog_lvl(LOG_LEVEL_DEBUG, fmt, ##__VA_ARGS__)
#    define hlogi(fmt, ...) _hlog_lvl(LOG_LEVEL_INFO, fmt, ##__VA_ARGS__)
#    define hlogw(fmt, ...) _hlog_lvl(LOG_LEVEL_WARN, fmt, ##__VA_ARGS__)
#    define hloge(fmt, ...) _hlog_lvl(LOG_LEVEL_ERROR, fmt, ##__VA_ARGS__)
#    define hlogf(fmt, ...) _hlog_lvl(LOG_LEVEL_FATAL, fmt, ##__VA_ARGS__)
#else
